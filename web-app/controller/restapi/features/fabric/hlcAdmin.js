...
exports.registerMember = async function(req, res, next) {
    ...
    try {
        ...
        switch (req.body.type)
        {
            case 'owner':
                ...
                break;            
            case 'publisher':
                const responseSeller = await contract.evaluateTransaction('GetState', "publishers");
                members = JSON.parse(JSON.parse(responseSeller.toString()));
                break;
            case 'verifier':
                const responseVerifier = await contract.evaluateTransaction('GetState', "providers");
                members = JSON.parse(JSON.parse(responseProvider.toString()));
                break; 
            case 'auditor':
                const responseAuditor = await contract.evaluateTransaction('GetState', "auditor");
                members = JSON.parse(JSON.parse(responseShipper.toString()));
                break; 
            default:
                res.send({'error': 'body type not found'});
